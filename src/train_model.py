...
LEARNING_RATE = 0.001
WEIGHT_DECAY = 1e-4
PATIENCE = 5  # Количество эпох без улучшения валидационных потерь для ранней остановки
...
# Трансформации данных с усиленной аугментацией
...
def plot_metrics(train_losses, val_losses, train_accs, val_accs, num_epochs, save_dir):
    ...
    plots_save_path = os.path.join(save_dir, "metrics_full_dataset.png")
...
if __name__ == '__main__':
    ...
    train_dataset = Food101Dataset(DB_PATH, "train", train_transforms)
    val_dataset = Food101Dataset(DB_PATH, "test", val_test_transforms)
    ...
    # Размораживаем последние 4 блока и классификатор
    ...
    # Инициализация AMP для оптимизации
    scaler = torch.amp.GradScaler(enabled=torch.cuda.is_available())

    # Параметры ранней остановки
    best_val_loss = float('inf')
    patience_counter = 0
    best_model_path = os.path.join(PROJECT_ROOT, "models", "best_efficientnet_b0_final.pth")
    ...
    for epoch in range(NUM_EPOCHS):
        ...
        if epoch_val_loss < best_val_loss:
            ...
            patience_counter += 1
            print(f"Эпохи без улучшения: {patience_counter}/{PATIENCE}")
            if patience_counter >= PATIENCE:
                print("Ранняя остановка: валидационные потери не улучшаются")
                break
    ...
    MODEL_SAVE_PATH = os.path.join(PROJECT_ROOT, "models", "efficientnet_b0_final.pth")
    if not os.path.exists(os.path.dirname(MODEL_SAVE_PATH)):
        os.makedirs(os.path.dirname(MODEL_SAVE_PATH))
    torch.save(model.state_dict(), MODEL_SAVE_PATH)
    print(f"Финальная модель сохранена по пути: {MODEL_SAVE_PATH}")

    # Визуализация метрик
    plots_dir = os.path.join(PROJECT_ROOT, "plots")
    if not os.path.exists(plots_dir):
        os.makedirs(plots_dir)
    plot_metrics(train_losses, val_losses, train_accs, val_accs, len(train_losses), plots_dir)
